...
- **Yearly Emissions** contains emissions data aggregated to the vessel level for each year. Each row represents one vessel and its yearly data. It also includes the annual CII score and rating. For the current year, the data reflects year-to-date values. **FuelEU Leg Part Emissions** contains additional FuelEU columns in addition to this dataset.
